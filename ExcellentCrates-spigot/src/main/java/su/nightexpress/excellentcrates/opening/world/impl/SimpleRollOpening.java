...
public class SimpleRollOpening extends WorldOpening {
    ...
    private void displayReward() {
        ...
        // Respawn rather than mutate: vanilla only force-syncs an Item entity's metadata to
        // clients once every 20 ticks, but spins can happen far more often than that (default
        // every 3 ticks). Mutating the existing entity's item/name left the visual pop lagging
        // behind the spin sound. A freshly spawned entity always sends its metadata immediately,
        // so respawning keeps the pop and the sound on the same tick.
        if (this.rewardDisplay != null) {
            this.rewardDisplay.remove();
        }
