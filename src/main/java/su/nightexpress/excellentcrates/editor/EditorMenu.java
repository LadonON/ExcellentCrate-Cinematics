...
public class EditorMenu extends NormalMenu<CratesPlugin> implements LangContainer {
    ...
    private static final IconLocale CINEMATICS_LOCALE = LangEntry.iconBuilder("Editor.Button.Main.Cinematics")
        .accentColor(SOFT_YELLOW)
        .name("Cinematics")
        .appendInfo("There are " + SOFT_YELLOW.wrap(GENERIC_AMOUNT) + " scenes created.").br()
        .appendInfo("Scripted camera scenes you can", "assign as a crate's opening animation.").br()
        .appendClick("Click to open").build();
    ...
    protected void onPrepare(@NotNull MenuViewer viewer, @NotNull InventoryView view) {
        ...
        viewer.addItem(NightItem.fromType(Material.ITEM_FRAME)
            .localized(CINEMATICS_LOCALE)
            .replacement(replacer -> replacer.replace(GENERIC_AMOUNT, () -> String.valueOf(this.plugin.getCinematicManager().countScenes())))
            .toMenuItem()
            .setSlots(13)
            .setHandler((viewer1, event) -> {
                this.runNextTick(() -> plugin.getEditorManager().openCinematicList(viewer.getPlayer()));
            })
            .build()
        );
    }
