...
public class CrateOptionsMenu extends LinkedMenu<CratesPlugin, Crate> implements LangContainer {
    ...
    private static final IconLocale LOCALE_LINKED_BLOCKS_NEXO = LangEntry.iconBuilder("Editor.Button.Crate.LinkedBlocksNexo")
        .name("Linked Block")
        .rawLore(DARK_GRAY.wrap("Press " + GOLD.wrap("[" + TagWrappers.KEY.apply("key.drop") + "]") + " to unlink.")).br()
        .appendCurrent("Linked", GENERIC_STATE).br()
        .appendInfo("Link the crate to a block by", "using the link tool.").br()
        .appendInfo("Interacting with the linked block", "will preview and open the crate.").br()
        .appendInfo("Drop a " + SOFT_YELLOW.wrap("Nexo item") + " on " + SOFT_YELLOW.wrap("this"), "button to get a tool that", "places that block and links", "the crate to it in one step.").br()
        .appendClick("Click to get link tool")
        .build();
    ...
    protected void onPrepare(@NotNull MenuViewer viewer, @NotNull InventoryView view) {
        ...
        viewer.addItem(NightItem.fromType(Material.BEACON)
            .localized(NexoHook.isInstalled() ? LOCALE_LINKED_BLOCKS_NEXO : LOCALE_LINKED_BLOCKS)
            ...
            .toMenuItem().setSlots(15).setHandler((viewer1, event) -> {
                ...
                // Dropping a Nexo item here upgrades the link tool into a "place and link" tool.
                // Without Nexo the branch is unreachable, since the hint is never shown.
                ItemStack cursor = event.getCursor();
                String nexoModel = null;
                if (cursor != null && !cursor.getType().isAir()) {
                    nexoModel = NexoHook.getIdFromItem(cursor);
                    Players.addItem(player, cursor);
                    event.getView().setCursor(null);
                }

                this.plugin.getCrateManager().giveLinkTool(player, crate, nexoModel);
