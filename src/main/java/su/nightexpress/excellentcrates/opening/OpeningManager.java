...
public class OpeningManager extends AbstractManager<CratesPlugin> {
    ...
    private void loadDefaults() {
        ...
        this.loadProvider("example", Config.DIR_OPENINGS_CINEMATIC, OpeningUtils::createCinematicExample);
    ...
    /**
     * Unregisters a provider, e.g. after its config file has been deleted from the editor.
     *
     * <p>Crates still pointing at the removed id fall back to the dummy provider in
     * {@link #createOpening(Player, CrateSource, Cost)}, so they keep working without an animation
     * instead of erroring.
     *
     * @return the removed provider, or {@code null} if no provider had that id.
     */
    @Nullable
    public OpeningProvider removeProvider(@NotNull String id) {
        return this.providerByIdMap.remove(id.toLowerCase());
    }
