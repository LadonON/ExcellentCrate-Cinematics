...
public class CrateUtils {
    ...
    /**
     * The cinematic editor's capture tool: right-clicking with it stores wherever the admin is
     * standing and looking, so camera and prop positions never have to be typed as coordinates.
     */
    @NotNull
    public static NightItem getDefaultCaptureTool() {
        return NightItem.fromType(Material.ENDER_EYE)
            .hideAllComponents()
            .setDisplayName(TagWrappers.GOLD.and(TagWrappers.BOLD).wrap("Capture Tool"))
            .setLore(Lists.newList(
                TagWrappers.GRAY.wrap("Stand where you want the"),
                TagWrappers.GRAY.wrap("position, look in the right"),
                TagWrappers.GRAY.wrap("direction, then right-click!")
            ));
    }
    @NotNull
